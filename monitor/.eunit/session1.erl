...
%%@@ @doc aw_ack_syn_ack
-spec aw_ack_syn_ack(Event :: timeout | #event{}, SDB :: #session_data{}) -> 
			R :: #next_state{}.
...
%----------------------------------
-spec established(Event :: timeout | #event{}, SDB :: #session_data{}) -> 
			R :: #next_state{}.
...
%%------------------------------
-spec aw_first_request(Event :: timeout | #event{}, 
		       SDB :: #session_data{}) -> 
			      R :: #next_state{}.
...
%%-----------------
-spec aw_first_response(Event :: timeout | #event{}, 
			   SDB :: #session_data{}) -> 
				  R :: #next_state{}.
...
aw_first_response(
  ...
  #session_data{sseq_next = Ack} = SD) ->
    #next_state{name = aw_first_response,
		session_data = rx(data_retrans(SD), L)};
...
%%--------------------------------------
-spec aw_request(Event :: timeout | #event{}, 
			   SDB :: #session_data{}) -> 
				  R :: #next_state{}.
...
aw_request(#s_pkt{rseq = Seq, l = L},
  ...
  when Seq < SSeqN->
    #next_state{name = aw_request,
		session_data = tx(data_retrans(SD), L)};
    
%% just ACK without Data  
%% from Client
aw_request(#c_ack{rseq = Seq, rack = Ack},
	   #session_data{cseq = Seq, sseq_next = SSeqN} = SD)
  when Ack =< SSeqN ->
    #next_state{name = aw_request,
		session_data =  SD};
...
-spec aw_response(Event :: timeout | #event{}, 
			   SDB :: #session_data{}) -> 
				  R :: #next_state{}.
...
aw_response(#s_ack{rseq = Seq, rack = Ack},
	    #session_data{sseq = Seq, cseq_next = CSeqN} = SD)
  when Ack =< CSeqN ->
    #next_state{name = aw_response,
		session_data =  SD};
...
%% @end
%%--------------------------------------------------------------------
-spec handle_event(Msg :: #msg{}, StateName :: atom(), SD :: #session_data{}) ->
			  R :: #next_state{}.
handle_event(
  #msg{sip = SIP, 
       ts = T, 
  ...
  #session_data{session = #session{client = C},
	 ...
	 sseq0 = SSeq0,
	 states = States,
	 pcap_records = PcapRecords} = SD) ->

    ClientPkt = (C =:= SIP),
    AckOnly = (DataLen =:= 0),

    {Seq0, Ack0} = seq(ClientPkt, CSeq0, SSeq0),
    RSeq = Seq - Seq0,
    RAck = Ack - Ack0,

    Event = if R =:= 1; F =:= 1 -> 
		    #session_end{cause = end_cause(ClientPkt, R)};
	       S =:= 1, A =:= 0 -> syn;
    ...
    NewState = SD#session_data{ states = [StateName | States],
			  t_last_msg = T,
			  pcap_records = [PcapRecord|PcapRecords]},
    
    R =  StateName(Event, NewState),
    R =/= unexpected orelse store_unexpected(Event, NewState),
    R.
...
sseq(#session_data{sseq_next = SSeqN} = SD, L) ->
    SD#session_data{cseq = SSeqN, cseq_next = SSeqN + L}.
...
-spec end_cause(ClientPkt :: boolean(), RST :: 0 | 1) -> end_cause().
...
store_terminate(Reason, StateName, SD) ->
   Descriptor = io_lib:format(
		  "session:~p with Session Data =~n~n~p~n~nReason=~n~n~p ", 
			       [StateName, SD, Reason]),
...
-spec seq(ClientPkt :: boolean(), CSeq0 :: seq(), SSeq0 :: seq()) ->
		 {Seq0 :: seq(), Ack0 :: seq()}.
seq(true, CSeq0, SSeq0) -> {CSeq0, SSeq0};
seq(_, CSeq0, SSeq0) -> {SSeq0, CSeq0}.
